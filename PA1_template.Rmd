...
activities <- read.table("activity.csv", header = T, sep = ",",
...
interval_summary <- aggregate(steps ~ interval, data = activities, mean,
...
The mean value of daily steps is **`r val_fmt(mean_steps)`**, and the median
value of daily steps is **`r val_fmt(median_steps)`**.
...
g + geom_line(color = "steelblue", size = 1.5) +
        labs(title = "Average Steps by Five-Minute Interval",
...
max_interval <- interval_summary[interval_summary$average_steps ==
        max_interval_steps,]$interval
```

The maximum value of average steps (**`r val_fmt(max_interval_steps)`**) occurs
in the  interval beginning at **`r val_fmt(max_interval)`**.
...
There are **`r val_fmt(missing_count)`** missing step values in the data set.
...
interval_median <- aggregate(steps ~ interval, data = activities, median,
        na.rm = T, na.action = na.pass)
names(interval_median) <- c("interval", "median_steps")
activities_imp <- merge(activities, interval_median, by = 'interval')
activities_imp <- activities_imp[order(activities_imp$date,
        activities_imp$interval),]
activities_imp[is.na(activities_imp$steps),]$steps <-
        activities_imp[is.na(activities_imp$steps),]$median_steps
...
```{r}
# create daily summary data frame
daily_summary_imp <- aggregate(steps ~ date, data = activities_imp, sum,
        na.rm = F, na.action = na.pass)
...
**`r val_fmt(mean_steps_imp)`**, and the median value is
**`r val_fmt(median_steps_imp)`**. Both of these values are lower than the
corresponding values excluding missing data (**`r val_fmt(mean_steps)`** and
**`r val_fmt(median_steps)`**, respectively). This is due to the combination of
...
**`r val_fmt(sum(interval_median$median_steps))`**, which can be seen in the
...
activities_imp$daytype <- factor(substr(weekdays(activities_imp$date),
        1, 1) == 'S', labels = c('weekday', 'weekend'))
```
...
```{r}
# create interval summary data frame
interval_summary_imp <- aggregate(steps ~ interval + daytype,
        data = activities_imp, mean, na.rm = T, na.action = na.pass)
...
g + geom_line(color = "steelblue", size = 1.5) +
        facet_grid(daytype ~ .) +
        labs(title = "Average Steps by Five-Minute Interval and Day Type",
        x = "Interval Start Time (hhmm)", y = "Average Steps")
```
